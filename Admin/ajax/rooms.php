...
if (isset($_POST['get_all_rooms'])) {
    ...
    while ($row = mysqli_fetch_assoc($res)) {
        $status = $row['status'] == 1
            ? "<button onclick='toggle_status($row[id],0)' class='btn custom-bg btn-sm shadow-none'>Active</button>"
...
if (isset($_POST['get_room'])) {
    ...
    $data = ["roomdata" => $room_data, "facilities" => $facilities];
...
if (isset($_POST['edit_room'])) {
    ...
    $del_facilities = update("DELETE FROM `room_facilities` WHERE `room_id`=?", [$frm_data['room_id']], 'i');
...
if (isset($_POST['get_room_images'])) {
    ...
    while ($row = mysqli_fetch_assoc($res)) {
        if ($row['thumb'] == 1) {
            $thumb_btn = "<i class='bi bi-check bg-success text-light px-2 py-1 rounded fs-5'></i>";
        } else {
...
if (isset($_POST['remove_room']))
 {
    ...
    $res2 = update("DELETE FROM `room_images` WHERE `room_id`=?", [$frm_data['room_id']], 'i');
    $res3 = update("DELETE FROM `room_facilities` WHERE `room_id`=?", [$frm_data['room_id']], 'i');
